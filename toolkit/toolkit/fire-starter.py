...
def sublist3r(args, home_dir, thisFqdn, logger):
    try:
        subprocess.run([f"python {home_dir}/Tools/Sublist3r/sublist3r.py -d {args.fqdn} -t 50 -o ./temp/sublist3r.tmp"], text=True, shell=True)
...
def subdomainizer(home_dir, thisFqdn, logger):
    try:
        ...
        subprocess.run([f"""timeout 4h python {home_dir}/Tools/SubDomainizer/SubDomainizer.py -l ./wordlists/live_servers.txt -o ./temp/subdomainizer.tmp -sop ./temp/secrets.tmp;if [ -f "./temp/secrets.tmp" ]; then cp ./temp/secrets.tmp /tmp; fi"""], shell=True)
        try:
...
def masscan_to_tls(home_dir):
    subprocess.run([f" masscan -p443 --rate 40000 -iL wordlists/aws_ips.txt -oL ./temp/clear_sky_masscan.tmp"], shell=True)
